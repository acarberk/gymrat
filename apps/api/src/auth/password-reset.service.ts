...
export class PasswordResetService {
  ...
  async forgotPassword(email: string): Promise<void> {
    ...
    try {
      await this.mailer.send({
        to: user.email,
        subject: 'Reset your Gymrat password',
        html,
        text,
      });
    } catch (error) {
  ...
  private renderHtml(displayName: string, url: string): string {
    ...
    return [
      ...
      `<p>Hi ${safeName},</p>`,
      `<p>Someone requested a password reset for your Gymrat account. If this was you, click the button below. The link expires in 1 hour.</p>`,
  ...
  private renderText(displayName: string, url: string): string {
    return [
      `Hi ${displayName},`,
      '',
      'Someone requested a password reset for your Gymrat account. If this was you, open the link below to set a new password. The link expires in 1 hour.',
      '',
      url,
      '',
